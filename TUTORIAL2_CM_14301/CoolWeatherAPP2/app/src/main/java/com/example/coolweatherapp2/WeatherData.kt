...
fun getWeatherDrawable(code: Int, isDay: Boolean): String {
    val map = getWeatherCodeMap()
    val weather = map[code]
    val imageName = weather?.image ?: "clear_"

    return if (imageName.endsWith("_")) {
        if (isDay) "${imageName}day"
        else "${imageName}night"
    } else {
        imageName
    }
}
