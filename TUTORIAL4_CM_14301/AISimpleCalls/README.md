...
- `OPENAI_API_KEY=...`
- `GEMINI_API_KEY=...`
- `AI_LLM=...` <span style="color: gray;">// Possible values: OPENAI, GEMINI, OPENAI-CLASSES, GEMINI-CLASSES; defaults to OPENAI</span>
- `TEMPERATURE=...` <span style="color: gray;">// Optional sampling temperature (e.g. 0.1-0.9); undefined uses each provider's default</span>
- `MAX_TOKENS=...` <span style="color: gray;">// Optional response length cap; undefined uses each provider's default</span>
- `MODE=...` <span style="color: gray;">// Possible values: CHAT, SENTIMENT; defaults to CHAT. SENTIMENT rates input on a 1-7 scale, returned as JSON</span>
