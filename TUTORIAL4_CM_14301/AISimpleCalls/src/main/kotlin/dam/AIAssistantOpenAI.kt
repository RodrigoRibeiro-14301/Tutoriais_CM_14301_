...
class AIAssistantOpenAI(override val properties: Properties) : AIAssistant {
    ...
    override fun buildRequest(prompt: String): Request {
        ...
        // Add temperature/max_tokens from TEMPERATURE/MAX_TOKENS properties, when defined
        temperature?.let { requestBodyJson.put("temperature", it) }
        maxTokens?.let { requestBodyJson.put("max_tokens", it) }
