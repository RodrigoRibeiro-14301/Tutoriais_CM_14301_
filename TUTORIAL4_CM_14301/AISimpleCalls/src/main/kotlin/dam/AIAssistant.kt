...
interface AIAssistant {
    ...
    /**
     * The sampling temperature to use for requests, read from the TEMPERATURE property.
     * May be undefined (null) in config.properties, in which case each implementation
    ...
    /**
     * The maximum number of tokens to generate, read from the MAX_TOKENS property.
     * May be undefined (null) in config.properties, in which case each implementation
     * falls back to its own default.
     */
    val maxTokens: Int?
    ...
    /**
     * Selects between plain chat input processing and sentiment analysis,
     * read from the MODE property. Possible values: CHAT, SENTIMENT.
     * Defaults to CHAT when undefined.
     */
    val mode: String
        get() = properties.getProperty("MODE", "CHAT").uppercase()
    ...
    /**
     * Builds a structured prompt for the model with consistent instructions.
     * Delegates to either the plain chat prompt or the sentiment analysis
     * prompt, depending on the configured MODE property.
     *
    ...
    fun buildPrompt(input: String): String {
        return if (mode == "SENTIMENT") buildSentimentPrompt(input) else buildChatPrompt(input)
    }

    /**
     * Builds the plain chat prompt, ensuring the model responds predictably
     * with a consistent, friendly personality.
     */
    fun buildChatPrompt(input: String): String {
        return """
    ...
    /**
     * Builds a prompt asking the model to rate the sentiment of the input
     * on a 7-point scale (1 = Very Negative ... 7 = Very Positive), and to
     * answer only with a JSON object containing the rating and a
     * justification, as required by Task 4 of the tutorial.
     */
    fun buildSentimentPrompt(input: String): String {
        return """
            Analyze the sentiment of the following text and rate it on this 7-point scale:
            1 = Very Negative
            2 = Negative
            3 = Slightly Negative
            4 = Neutral
            ...
            {
              "rating": <integer 1-7>,
              "justification": "<short explanation for the rating>"
            }
